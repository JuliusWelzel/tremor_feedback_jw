...
def single_trial_force_raw(eps,id):
    ...
    fig.tight_layout()
    sns.despine(fig=fig)
    fig.savefig(Path.joinpath(dir_plots_single_trial,f"{id}_fsr_epochs.png"))
    fig.clf()
...
def single_trial_specs(eps, id):
    ...
    fig.savefig(Path.joinpath(dir_plots_single_trial, f"{id}_single_trial_trmr.png"))