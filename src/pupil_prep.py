...
def confidence_threshold(data, conf, thr, doplot):
    ...
    p_subthresh = 1.0 * len(idx_subthresh) / len(data)
...
def delete_outliers(data, whsize, n_mad, doplot):
    ...
    p_outlier = 1.0 * len(outlier_idx) / n
    ...
    if doplot:
        ...
        plt.plot(data, "-k")
        plt.plot(
            outlier_idx, data[outlier_idx], "or", ms=8, alpha=0.5, label="outliers"
        )
        plt.legend()
